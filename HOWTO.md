...
| `IRIS_AI_MAX_CONTEXT_TOKENS` | 60 000 | Estimated context ceiling; triggers compaction, not a stop. The estimate counts the tool schemas (~13k tokens for the 58-tool registry) and the system prompt as well as the transcript. The schemas ride on EVERY request and no fold can reach them, so when they are more than a third of the window their descriptions are shortened once (~3k tokens freed); every tool and every parameter stays available. The model's OWN window is the real ceiling: when the provider refuses a request for its size (HTTP 400 naming the context), Iris folds the transcript, sets this run's ceiling from the limit the provider stated (or below what was refused) and retries the same turn. When folding cannot fit, the run restarts from its own record — the same recovery as typing "continue" — up to 3 times before it stops. |
...
| `IRIS_AI_WORKER_STEPS` | 10 (cap 30) | Model turns ONE delegated worker agent may take. A worker answers one scoped question, so this is deliberately small; the lead's own budget is unaffected. |
| `IRIS_AI_WORKER_SECONDS` | 300 (cap 900) | Wall clock one worker agent gets. It is also bounded by what is left of the `delegate_investigation` call's own budget, so it can never outlive the call waiting for it. |
| `IRIS_AI_WORKER_READS` | 3 (cap 4) | How many of ONE worker's own tool calls run at the same time. |
