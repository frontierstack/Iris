...
| **Detections** | 120 Sigma-like built-ins — 108 per-event, plus 12 that read the entity graph — across web, identity, Windows, Linux, AWS, Azure/Entra ID, Microsoft 365, Defender, Kubernetes, mail and packet captures. Every constant is editable, and a custom rule is a regex or a set of typed conditions. Each one carries a second opinion where a bare pattern would be noisy: a `README.txt` is not ransomware, `whoami` is not an attack, and a verdict the product already BLOCKED is reported as contained rather than as an incident. |
...
| **AI investigator** | One free-form objective. The assistant drives Iris's own tools and streams what it does. It FOLLOWS THREADS — one call walks the connections several hops out and returns the chains, ranked by how specific each link is rather than how busy — and Iris keeps a ledger of what it has already asked and what it has not yet looked at, so a small model does not re-run its own work after the conversation is folded to fit its context window. Every finding is cited to a real event id, and every change it makes is listed and reversible. |
...
| **Appearance** | 18 themes grouped by what they are for — dark, light, and two high-contrast (black and white) — each previewed as a miniature of the app, with a filter over names, descriptions and tags. Three typeface axes: the interface, the monospace face every log line is set in, and the assistant's reading face. Everything is bundled; nothing is fetched from the internet at runtime. |
