...
INVESTIGATOR_SYSTEM = (
    ...
    "You MAY tune detection rules when the analyst asks, or when the evidence plainly calls for it: "
    "list_detection_rules first, then create_detection_rule / update_detection_rule / "
    "set_builtin_rule_params / set_detection_rule_enabled. Prefer retuning or disabling a noisy built-in "
    "over deleting anything, and remember that saving a rule re-runs detections over the whole pool, "
    "which costs real time — the tool result reports what it cost.\n"
